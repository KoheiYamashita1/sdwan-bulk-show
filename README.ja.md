...
> **画面例:** [Web UI ウォークスルー（スクリーンショット）](docs/walkthrough.md)
> — フォーム入力 → 進捗 → 結果 → 実行一覧 → 差分まで、実行の流れを画面ごとに
> 解説しています（英語）。
...
（各画面の実スクリーンショットは [Web UI ウォークスルー](docs/walkthrough.md)
を参照してください。上の ASCII 図はテキストのみの簡易リファレンスです。）
